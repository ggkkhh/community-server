...
/**
 * @USER: roydon
 * @Description 身份证加密 4114**********7517
 */
public class IdCardNumUtil {
    private static final int ID_CARD_PLAIN_TEXT_LENGTH = 4;
    ...
    public static boolean validate18Idcard(String idcard) {
        ...
        int s = 18;
        if (idcard.length() != s) {
//            LOG.warning(BSModule.USER_SERVICE, "身份证位数不正确!");
            return false;
        }
        // 获取前17位
    ...
    /**
     * 使用星号替换身份证号的中间部分，只显示前4位和后4位
     *
     * @param idCard
     * @return
     */
    public static String replaceSomeCharByAsterisk(String idCard) {
        if (StringUtil.isEmpty(idCard)) {
            return idCard;
        }
        char[] chars = idCard.toCharArray();
        for (int i = ID_CARD_PLAIN_TEXT_LENGTH; i < chars.length - ID_CARD_PLAIN_TEXT_LENGTH; i++) {
            chars[i] = ASTERISK;
        }
    ...
    public static void main(String[] args) throws Exception {
        String idcard15 = "372922765402237726";
        String idcard18 = "411023199102206038";//
        String idcard = "440221200005012715";
        // 自己身份证测试
